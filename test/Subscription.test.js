...
describe("Subscription (generations above 1st)", function () {
  ...
  it("Subscribe for 5000$", async function () {
    ...
    expect(await usdt.balanceOf(owner.address)).to.be.closeTo(
      "99999399999999949063000000", parseEther("1")
    )
  });
  it("Withdraw", async function () {
  ...
  it("Mint gen0 tokens", async function () {
    await stackOsNFT.partnerMint(4);
  });
  it("Subscribe multiple tokens at once", async function () {
    ...
    let path = [usdt.address, await router.WETH(), stackToken.address];
    let contractWillReceive = await router.getAmountsOut(
      parseUnits("500", 6),
      path
    );

    let oldBalance = await usdt.balanceOf(owner.address);
    let oldContractBalance = await stackToken.balanceOf(sub0.address);
    await sub0.subscribe(0, [...Array(5).keys()], parseEther("100"), usdt.address, false); 
    let newBalance = await usdt.balanceOf(owner.address);
    let newContractBalance = await stackToken.balanceOf(sub0.address);
    
    stackTokensReceived = newContractBalance.sub(oldContractBalance);
    expect(stackTokensReceived).to.be.closeTo(
      contractWillReceive[path.length-1], parseEther("0.1")
    );
    expect(oldBalance.sub(newBalance)).to.be.equal(parseUnits("500", 6));
  });
  it("Withdraw", async function () {
    let oldBalance = await stackToken.balanceOf(owner.address);
    await sub0.withdraw(0, [...Array(5).keys()]);
    let newBalance = await stackToken.balanceOf(owner.address);
    expect(newBalance.sub(oldBalance)).to.be.closeTo(
      stackTokensReceived.mul(25).div(100), parseEther("1")
    )
  });
