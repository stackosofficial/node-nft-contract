...
contract BlackMatter is ERC721, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    mapping(address => uint256) private deposits; // total tokens deposited, from any generation
    mapping(address => uint256) private lastUserBlackMatter; // owner => current incomplete blackmatter id
    mapping(address => uint256[]) private toBeMinted; // owner => BlackMatterNFT ids to be minted
    mapping(uint256 => mapping(uint256 => uint256)) private stackToMaster; // generation => stackNFT id => blackmatter id

    GenerationManager private generations;

    uint256 immutable mintPrice; // number of StackNFTs that must be deposited in order to be able to mint a BlackMatter.

    constructor(
        GenerationManager _generations,
        uint256 _mintPrice
    ) ERC721("BlackMatter", "MN") {
        generations = _generations;
        mintPrice = _mintPrice;
    }

    /*
     * @title Returns true if StackNFT token is locked in BlackMatter.
    ...
    /*
     * @title Returns true if `_wallet` own either StackNFT or BlackMatterNFT that owns StackNFT.
     * @param Token holder address.
     * @param StackNFT generation id.
     * @param StackNFT token id.
     */
    function isOwnStackOrBlackMatter(address _wallet, uint256 generationId, uint256 tokenId) public view returns (bool) {
     ...
     * @param StackNFT token id.
     */
    function ownerOfStackOrBlackMatter(IStackOSNFT _stackOsNFT, uint256 tokenId) public view returns (address) {
    ...
    /*
     * @title Returns owner of the BlackMatterNFT that owns designated StackNFT.
     * @param StackNFT generation id.
     * @param StackNFT token id.
     */
    ...
    function deposit(uint256 generationId, uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(deposits[msg.sender] == 0) {
                lastUserBlackMatter[msg.sender] = _tokenIdCounter.current();
                _tokenIdCounter.increment();
            }
            deposits[msg.sender] += 1;
            if(deposits[msg.sender] == mintPrice) {
                deposits[msg.sender] -= mintPrice;
                stackToMaster[generationId][tokenId] = lastUserBlackMatter[msg.sender];
                toBeMinted[msg.sender].push(lastUserBlackMatter[msg.sender]);
            } else {
                stackToMaster[generationId][tokenId] = lastUserBlackMatter[msg.sender];
            }
        }
    ...
    /*
     *  @title Mints a BlackMatterNFT for the caller.
